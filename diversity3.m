...
file_path = 'sim1/snapshots/Dx_10/Barrier_SIR_step_10000.mat'; % Change this!
% Load the data
...
for local_d = 1:num_selected
    for b = 1:num_bins
        ...
        if c > 0
            ...
            % Width proportional to number of particles (Max width = 0.8)
            width = 0.8 * (c / max_count);
            
            % Left-aligned patches (starting from local_d - 0.4)
            x_left = local_d - 0.4;
...
% Overlays (Weighted mean, True State)
...
% Overlay Observed Value ONLY if current step is an exact observation time
if isfield(snapshot, 'ze_sparse') && isfield(snapshot, 'n_obs') && ~isempty(snapshot.ze_sparse)
    % Check if this step is a multiple of n_obs (meaning an observation just arrived)
    if step_idx > 0 && rem(step_idx, snapshot.n_obs) == 0
        obs_col = step_idx / snapshot.n_obs; % Get the exact observation index
